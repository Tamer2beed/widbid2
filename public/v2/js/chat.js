...
function renderOnlineUsers() {
    ...
    sortedUsers.forEach(user => {
        ...
        const nameLabel = isMe ? `<span class="font-extrabold${glowClass}"${nameStyle}>${sanitize(user.name)}</span> <span class="text-purple-500 text-[9px] font-normal">(أنت)</span>` : `<span class="font-extrabold"${nameStyle}>${sanitize(user.name)}</span>`;
